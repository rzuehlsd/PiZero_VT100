...
- [Manual Verification Plan (2026-02-12 to 2026-02-14)](#manual-verification-plan-2026-02-12-to-2026-02-14)
  ...
  - [A2) WLAN Log Mode Strict Command Surface](#a2-wlan-log-mode-strict-command-surface)
...
## A2) WLAN Log Mode Strict Command Surface

### Purpose of Test

Verify that log mode does not allow manual switching into host bridge mode.

| Step No | Description | Expected Result | Verification |
| --- | --- | --- | --- |
| 1 | In telnet command mode, run `host on`. | Command is rejected / treated as non-built-in; session remains in log mode. | PASS / FAIL / N/A |
| 2 | Verify prompt behavior after the command. | Prompt remains in log mode (`>: `) and no host bridge is entered. | PASS / FAIL / N/A |
| 3 | Run `help`. | Only log-mode command set is documented (`help`, `status`, `echo`, `exit`). | PASS / FAIL / N/A |
...
| 2 | Reconnect via telnet. | Host mode is active immediately after connect, without visible startup control-byte artifacts in VT100 output. | PASS / FAIL / N/A |
| 3 | Send first host input immediately after connect (for example `dir` + `CR`). | First command is processed directly (no extra warm-up `CR` required). | PASS / FAIL / N/A |
| 4 | Set `wlan_host_autostart=0`, reboot, reconnect. | Session starts in command/log mode. | PASS / FAIL / N/A |
...
| 2 | Set `wlan_host_autostart=1`, reconnect via telnet. | TCP host path becomes active immediately after connect. | PASS / FAIL / N/A |
| 3 | Observe display source behavior in host mode. | Mixed UART/TCP rendering is avoided. | PASS / FAIL / N/A |
| 4 | Disconnect TCP host client and reconnect with `wlan_host_autostart=0`. | Session returns to log mode and normal UART rendering behavior. | PASS / FAIL / N/A |
